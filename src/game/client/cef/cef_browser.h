 ...
 * cef_browser.h, Main CEF browser. Use this to render the browser in your VGUI panels.
 *
 * Licensed under CC BY-NC 3.0 (Lambda Wars' original license)
 */

#ifndef CEF_BROWSER_H
#define CEF_BROWSER_H
#ifdef _WIN32
#pragma once
#endif // _WIN32
...
#define USE_MULTITHREADED_MESSAGELOOP 1
...
{
public:
	CefClientHandler(CCefBrowser* pSrcBrowser, CefNavigationType navigationbehavior, const char* pDebugName);
	...
	// CefRenderHandler
	void SetOSRHandler(CefRefPtr<CCefOSRRenderer> handler) {
		m_OSRHandler = handler;
	}
	CefRefPtr<CCefOSRRenderer> GetOSRHandler() { return m_OSRHandler; }
...
private:
	...
	CefRefPtr<CCefOSRRenderer> m_OSRHandler;
	...
	// Navigation behavior
	CefNavigationType m_NavigationBehavior;

	// Internal
	CCefBrowser* m_pSrcBrowser;
	...
	typedef struct messageData_t {
		messageData_t() : type(MT_UNKNOWN), frame(nullptr), data(nullptr) {}
...
public:
	CCefBrowser(
		...
		int wide = 0,
		int tall = 0,
		CefNavigationType navigationbehavior = NT_DEFAULT);
	~CCefBrowser();
	...
	CefRefPtr<JSObject> CreateFunction(const char* name, CefRefPtr<JSObject> object = nullptr, bool bHasCallback = false);
	...
	// Internal
	CCefVGUIPanel* GetPanel() { return m_pPanel; }
	vgui::VPANEL GetVPanel();
	CefRefPtr<CCefOSRRenderer> GetOSRHandler();
	CefRefPtr<CefBrowser> GetBrowser();
	const char* GetName();

protected:
	CCefBrowser(CCefBrowser const& vec) { Assert(0); }
...
inline void CCefBrowser::SetGameInputEnabled(bool state)
{
	m_bGameInputEnabled = state;
}

inline bool CCefBrowser::IsGameInputEnabled()
{
	return m_bGameInputEnabled;
}

inline const char* CCefBrowser::GetName()
{
	return m_Name.c_str();
}

inline bool CCefBrowser::IsAlphaZeroAt(int x, int y)
{
	return GetAlphaAt(x, y) == 0;
}

inline float CCefBrowser::GetLastLoadStartTime()
{
